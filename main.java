...
/** Hệ thống chẩn đoán y tế cho bệnh tim
*/
import javax.swing.*;
...
public class main<LogicServer> extends JFrame implements TreeSelectionListener, mainInterface {
	...
	public main() {
		...
		setBounds((paintx - 610) / 2, ((painty - 100) - 680) / 2, 610, 680);
		...
		exit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirm = JOptionPane.showConfirmDialog(null, "Are you sure want to exit?", "Exit Application",
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			if (s.equals("Add") || s.equals("Delete") || s.equals("Update") || s.equals("Search")) {
				...
				infoPanel.add(p);
				infoPanel.validate();
			} else if (s.equals("Diagonise")) {
				runKnow();
			} else if (s.equals("Congenital Heart Disease")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchCongestive();
				infoPanel.validate();
			} else if (s.equals("Aortic Aneurysm")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchAortic();
				infoPanel.validate();
			} else if (s.equals("Angina")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchAngina();
				infoPanel.validate();
			} else if (s.equals("Modify Facts")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				infoPanel.add(addRules);
				infoPanel.validate();
			} else if (s.equals("Info on Heart")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchHeart();
				infoPanel.validate();
			} else if (s.equals("Calculate your angina risk factor")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				infoPanel.add(r_f);
				infoPanel.validate();
				...
				launchContact();
				infoPanel.validate();
			} else if (s.equals("Patient Guide")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchBasic(1);
				infoPanel.validate();
			} else if (s.equals("Heart - A Strong Muscle")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchBasic(2);
				infoPanel.validate();
			} else if (s.equals("Treatment for Aortic Aneurysm")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchTreatment(1);
				infoPanel.validate();
			} else if (s.equals("Treatment for Angina")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchTreatment(2);
				infoPanel.validate();
			} else if (s.equals("Treatment for Congenital Heart Disease")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchTreatment(3);
				infoPanel.validate();
			} else if (s.equals("Know Symptoms")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchLink(1);
				infoPanel.validate();
			} else if (s.equals("Control your risk factors")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchLink(2);
				infoPanel.validate();
			} else if (s.equals("Excercise")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchLink(3);
				infoPanel.validate();
			} else if (s.equals("Learn lifetime weight control")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchLink(4);
				infoPanel.validate();
			} else if (s.equals("Discover new developments")) {
				infoPanel.removeAll();
				infoPanel.repaint();
				launchLink(5);
				infoPanel.validate();
	...
	void runKnow() {
		...
		try {
			...
			// The session_directory is where a temporary file is created to maintain the
			// state
			// of the reasoning process
			// The log_file is a log of the reasoning process and is created in the
			// directory
			...
			term = ls.ExecStr(s);
			if (term == 0)
				JOptionPane.showMessageDialog(null, "kwi / initialize failed", "Error", JOptionPane.ERROR_MESSAGE);
			System.out.print("8");
         ...
         outputText.setText("System: " + engineName + " Version: " + version);
			...
			// String name = JOptionPane.showInputDialog("Enter name of knowledgebase to
			// open:");
			if (name.length() == 0)
				return;
			...
			if (ls.ExecStr("kwi('" + id + "', new_session, _)") == 0)
				JOptionPane.showMessageDialog(null, "kwi / new_session failed", "Error", JOptionPane.ERROR_MESSAGE);
			...
			while ((solveTerm = ls.ExecStr("kwi('" + id + "', solve, _MORE)")) != 0) {
				...
				do {
					...
					if ((actionTerm = ls.ExecStr("kwi('" + id + "', get_action, _ACTION)")) == 0) {
						System.out.print("12");
						JOptionPane.showMessageDialog(null, "kwi / get_action", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
					...
					if (responseFunctor.equals("ask") && responseType.equals("user")) {
						...
						if (ask.getSelectedValue() == null)
							return;

						// Assert the new fact or facts (for multiple choices)
						if (oneAnswer)
							s = "kwi('" + id + "', assert(fact('" + factName + "', $" + ask.getSelectedValue()
									+ "$)), _)";

						// Multiple answers need to be asserted as a prolog list which is enclosed in
						// []'s
						else {
							...
							s = "kwi('" + id + "', assert(fact('" + factName + "', ["
									+ factList.substring(0, factList.length() - 1) + "])), _)";
						}
						if (ls.ExecStr(s) == 0)
							JOptionPane.showMessageDialog(null, "kwi / assert", "Error", JOptionPane.ERROR_MESSAGE);
						// }
					}
					...
					if (responseFunctor.equals("tell") && responseType.equals("user")) {
						...
						if (newSolution != null) {
							...
							JOptionPane.showMessageDialog(null, "No text for goal: " + answerName, "Error",
